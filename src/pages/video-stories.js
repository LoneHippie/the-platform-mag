...
import { Layout, PostContentLayout, Seo, VideoCardStandard } from '../components';
                ...
                <section className={classes.stories_section}>
                    {
                        categories.map((el, index) => (
                            <section className={classes.stories_section_category} key={`section-${index}`}>
                                <h2>{el}</h2>
...
export const pageQuery = graphql`
    query {
        stories: allContentfulStories(sort: {fields: createdAt}, filter: {node_locale: {eq: "en-US"}}) {
            edges {
                node {
                    youtubeUrl
                    title

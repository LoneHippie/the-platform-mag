  ...
  return (
    <Layout darkNavIcons={true} darkFooterBackground={true}>
      <Seo
        ...
        description={
          "Exclusive article available only on The Platform Mag, read the full article here"
        }
        type="article"
      />

      <PostContentLayout
        includeRecentEditionsBar={!data.article.isPartOfEdition}
      >
        <section
          className={!data.article.isPartOfEdition ? classes.main_content : ""}
        >
          <img
          ...
          <div className={classes.meta_section}>
            <img
              ...
              src={
                data.article.authorImage
                  ? `https:${data.article.authorImage.file.url}`
                  : Placeholder
              }
            ...
            <div className={classes.meta_details}>
              ...
              <h3>
                {data.article.isPartOfEdition
                  ? data.article.postDate.slice(3)
                  : data.article.postDate}
              </h3>
            </div>
          </div>

...
const EditionCardMini = ({ edition }) => {
  ...
  return (
    <article className={classes.card}>
      <span className={classes.card_edition}>
        {edition.slug.replaceAll(/-/g, " ")}
      </span>
      <div className={classes.card_body} style={{ background: editionCover }}>
        <a href={`/${edition.slug}`} alt="Read the full edition">
          {edition.editionTitle}
        </a>
      </div>
      <span className={classes.card_date}>
        {formattedEditionMonth(edition.editionMonth)}
      </span>
    </article>
  );
};
